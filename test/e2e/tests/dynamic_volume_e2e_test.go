...
//
// Test flow:
//  1. Pick an n2d-standard-4 VM (testContexts) — these do NOT support hyperdisk
...
var _ = Describe("GCE PD CSI Driver Dynamic Volumes PD Fallback Provisioning", func() {
	...
	It("Should fall back to pd-balanced on PD-only node when type=dynamic", func() {
		...
		// Dynamic volume params: driver selects HD or PD based on node topology labels
		...
		defer func() {
			// Cleanup: Delete volume and verify deletion
		...
		// Verify via GCE API that the disk was created with pd-balanced.
		// The driver must resolve "dynamic" → "pd-balanced" on PD-only n2d nodes.
