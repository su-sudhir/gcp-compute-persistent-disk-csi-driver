...
// TC-01: HD Node Dynamic Provisioning
//
...
//
// Test flow:
//  1. Pick a c3-standard-4 VM (hyperdiskTestContexts) — these support hyperdisk-balanced
...
var _ = Describe("GCE PD CSI Driver Dynamic Volumes", func() {
	...
	It("Should provision hyperdisk-balanced on HD-capable node when type=dynamic", func() {
		...
		// Dynamic volume params: driver selects HD or PD based on node labels
		params := map[string]string{
			parameters.ParameterKeyType: parameters.DynamicVolumeType, // "dynamic"
			parameters.ParameterHDType:  "hyperdisk-balanced",         // preferred on HD-capable nodes
			parameters.ParameterPDType:  "pd-balanced",                // fallback for PD-only nodes
		}
		...
		defer func() {
			// Step 4: Delete volume and verify deletion
		...
		// Step 2: Verify via GCE API that the disk was created with hyperdisk-balanced
		// The driver must resolve "dynamic" → "hyperdisk-balanced" on c3 nodes
		...
		// Step 3: The resolved disk type must be hyperdisk-balanced, NOT "dynamic" or "pd-balanced"
